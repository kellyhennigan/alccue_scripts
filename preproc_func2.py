...
# variables for transforming functional data to group space
vox_dim = 2.9 # voxel dimensions of functional data (mm3 isotropic)
t1_tlrc_afni_file = 't1_tlrc_afni.nii.gz' # t1 in tlrc space, using afni xform
xf_afni = 'xfs/'+task+'2tlrc_xform_afni' # functional native space > tlrc xform 
print 'apply func2t1 coregistration?'
doCoreg = bool(input('enter 1 for yes, or 0 for no: '))
	

# filepaths for ROI masks
roiPath = os.path.join(dataDir,'ROIs','%s_func.nii') # %s is roiNames
...
def normalizeAfniFunc(inFile):
	...
	# uses global variables t1_tlrc_afni_file, xf_afni, vox_dim, & doCoreg
...
if __name__ == '__main__':
	...
	for subject in subjects:  	# subject loop
		...
		funcFile = cat4d(ppFiles,'pp_'+task)
		...
		########## transform pre-processed functional data to group space
 		funcFile = normalizeAfniFunc(funcFile)
		...
		for r in roiNames:
			...
			extractRoiTS(funcFile,this_roiPath,outTsName)
		...
		print('\n\npre-processed '+task+' data saved to '+funcFile+'\n\n')
